            ...
            <div class="modal-footer">
                ...
                <?php if($user_role != 'Super Administrator'){ if($data['outstanding'] != 0 ){ ?>
                    <!-- <span class="button-approve-mutasi"> -->
                        <button type="button" class="btn btn-flat label-danger button-approved-mutasi" id="button-approved-mutasi" OnClick="changeStatusMutasi('A')" style="margin-right: 5px;">APPROVE</button>
                    <!-- </span> -->
...
<script>
    ...
    function changeStatusMutasi(status)
    {
        ...
        var input3 = document.getElementsByName('po_type[]'); 
        var mandatory_ac = [];    
        var no_reg_item = [];    
        var po_type = [];    
        var kode_sap = [];    
        ...
        for (var i = 0; i < input.length/2; i++) {             
            ...
            jenis_asset_tujuan.push(jns_at);
                ...
                {
                    // AMP & LAIN
                    var kode_asset_nilai = $("#request-form #kode_asset_ams_tujuan-"+no_reg_item[i]+"").val();
                }
                else
                {
                    // SAP
                    var kode_asset_nilai = $("#request-form #kode_sap_tujuan-"+no_reg_item[i]+"").val();
                }
                kode_sap.push(kode_asset_nilai);
        ...
        <?php } ?>    
            ...
            if(confirm('confirm '+status_desc+' data ?'))
            {
                ...
                var request_ka = JSON.stringify(request_kode_aset_data);
                var request_gi = JSON.stringify(request_check_gi);
                var kac = JSON.stringify(kode_ac);
                var ksap = JSON.stringify(kode_sap);
                //alert(param); //return false;
