...
class RequestAsetLainController extends Controller
{
    public function index()
    {
        echo "Aset Lainnya";die();
    ...
    {
        //echo "Create - Aset Lainnya";die();
        if (empty(Session::get('authenticated')))
            return redirect('/login');

        $data['ctree_mod'] = 'Pendaftaran';
        
        $data['page_title'] = 'Request Aset Lainnya';
        $data['type'] = 'Aset Lain';
        $access = AccessRight::access();
        $data["access"] = (object)$access;
        ...
        $data['ctree'] = 'create/aset-lain';

        //echo "<pre>"; print_r($data['ba_user']); die();
        return view('request.aset_lain')->with(compact('data'));
    }
    ...
    public function store(Request $request)
    {
       ...
       try 
       {
            ...
            DB::STATEMENT('call create_approval("P3", "'.$request->business_area.'","","'.$reg_no.'","'.$user_id.'","'.$asset_type.'","")');
            //die();
